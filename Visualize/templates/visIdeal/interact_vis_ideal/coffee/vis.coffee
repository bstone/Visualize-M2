...
Plot = () ->
  ...
  pointData = []
  rectData = []
  xScale = d3.scale.linear().domain([minScale,maxXScale]).range([0,width])
  #yScale = d3.scale.linear().domain([0,10]).range([0,height])
  yScale = d3.scale.linear().domain([minScale,maxYScale]).range([height,0])
  ...
  chart = (selection) ->
    selection.each (rawData) ->
      ...
      [minScale..maxXScale].forEach (x) ->
        [minScale..maxYScale].forEach (y) ->
          pointData.push([x,y])
